    ...
    <section>
      <Element name="features">
        <div className="container">
          ...
          >
            {features.map(
              ({ id, icon, title, caption, text, button }: FEATURES) => (
                <div
                  key={id}
                  className="relative z-2 md:px-10 px-5 mb:pb-10 pb-5 flex-50 max-md:g7 
                ...
                >
                  ...
                  <Button icon={button.icon} onClick={() => {}}>
                    {button.title}
                  </Button>
                </div>
              )
            ...
            <ul className="relative flex justify-around flex-grow px-[5%] border-2 border-s3 rounded-7xl max-md:hidden">
              ...
              {details.map(({ id, icon, title }: DETAILS) => (
